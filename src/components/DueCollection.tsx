...
import { CreditCard, Plus, Check, DollarSign, Store } from 'lucide-react';

export default function DueCollection() {
  const { sellers, addDueEntry, markDueAsPaid, getSellerDues, getSellerTotalDue } = useInventory();
  ...
  const handleMarkAsPaid = (dueId: string, shopName: string, amount: number) => {
    if (confirm(`Mark due from ${shopName} ($${amount.toFixed(2)}) as paid?`)) {
      markDueAsPaid(dueId);
    }
  };

  const selectedSellerDues = selectedSeller ? getSellerDues(selectedSeller) : [];
  const selectedSellerTotalDue = selectedSeller ? getSellerTotalDue(selectedSeller) : 0;
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="bg-orange-50 px-4 py-2 rounded-lg border border-orange-200">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-lg font-bold text-orange-800">${selectedSellerTotalDue.toFixed(2)}</span>
                </div>
              </div>
            </div>

            {selectedSellerDues.length === 0 ? (
                    ...
                    {selectedSellerDues.map((due) => (
                      <tr key={due.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          <button
                            onClick={() => handleMarkAsPaid(due.id, due.shopName, due.dueAmount)}
